...
GIVEN("A bombe using wheels I, II, III and reflector UKW"){
    ...
    WHEN("Given a long ciphertext encrypted with ring-setting AAA, ring-position AAA and no steckering") {
        string plaintext ="THISISAPLAINTEXTTOBEENCRYPTEDWITHTHEENIGMAANDISVERYVERYLONG";
        string ciphertext=enigma.encrypt(plaintext);
        enigma.reset();
        THEN("Running bombe with a complete crib should return the above setting") {
